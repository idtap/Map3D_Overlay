...
//        }, 5000);        
    ...
    private void initViewEventHadler() {
        mapView.setOnMouseMoved(e -> {
            ...
            if (null != sketchEditorFE && (null != sketchEditorFE.getSelectedGraphic()
                    || (null != milSymbolPicker && milSymbolPicker.isCurveLine())))
...
//                  editLocationPane.setPointCollection(multiPointCollection);
    ...
    private void handleAnalysisRoute(ActionEvent e) {
        ...
        // 經過點、障礙物、路徑圖層加入
        mapView.getGraphicsOverlays()
                .addAll(Arrays.asList(stopsGraphicsOverlay, barriersGraphicsOverlay, routeGraphicsOverlay));

        // 地點圖標載入
        pinImage = new Image(getClass().getResourceAsStream("/icons/orange_symbol.png"), 0, 40, true, true);
            ...
            if (event.isStillSincePress()) {
                ...
                if (event.getButton() == MouseButton.PRIMARY) {
                    ...
                    if (analysisRouteControllerHandle.btnAddStop_isSelected()) {
                        ...
                        // 依障礙物大小
                        Polygon bufferedBarrierPolygon = GeometryEngine.buffer(mapPoint,
    ...
    private CompositeSymbol createCompositeStopSymbol(Integer stopNumber) {
        TextSymbol stopTextSymbol = new TextSymbol(16, (stopNumber).toString(), 0xFFFFFFFF,
    ...
    private void startRouteTask() {
        ...
        mobileMapPackage.addDoneLoadingListener(() -> {
            if (mobileMapPackage.getLoadStatus() == LoadStatus.LOADED && !mobileMapPackage.getMaps().isEmpty()) {
                ...
                // Win20231220, 此段正式版可能要關閉以避免與 vtpk 重複顯示
                mapView.getMap().getOperationalLayers().add( dataRoadLayer );

                // 開啟 routeTask 後續作業
                // 取下 ND dataset
                TransportationNetworkDataset networkDataset = mobileMapPackage.getMaps().get(0)
                        .getTransportationNetworks().get(0);
                        ...
                        if (routeTask.getLoadStatus() == LoadStatus.LOADED) {
                            ...
                            if (routeParameters == null) {
                                try {
                                    ...
                                    analysisRouteControllerHandle.switchTravelMode();
    ...
    private void analysisRouteOpen() {
        try {
            ...
            analysisRouteFrame.setSize(270, 600);
                    ...
                    if (!routeResult.getRoutes().isEmpty()) {
                        ...
                        if( analysisRouteControllerHandle.optSpeed_isSelected() )
                            output = String.format("分析結果: %d 分鐘 (%.2f 公里)", 
                                Math.round(firstRoute.getTotalLength() / 1000 * 60 / analysisRouteControllerHandle.getMoveSpeed()),
                                firstRoute.getTotalLength() / 1000);
